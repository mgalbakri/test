...
def write_summary():
    ...
    A('This machine has 4 CPU cores and no GPU. Measured on the master '
      'bedroom: a 1920x1080 draft frame takes **162-168 s**, and a '
      '3840x2160 final **954-983 s**. One room is 4 views, so about '
      '**11 minutes of draft and 64 minutes of final per room**.\n')
    A('At nine rooms that is about **1.7 hours of draft and 9.6 hours of '
      'final** rendering, before any re-render after QA - and this room '
      'needed a full re-render when a material fault showed up only at 4K. '
      'Frames are time-boxed (Cycles `time_limit`, with auto-tiling off so '
      'the cap is per frame rather than per tile) and the denoiser carries '
      'the remainder. At 3840x2160 the cap binds around 45 samples, so broad '
      'surfaces and lighting hold up but the finest grain is softened. That '
      'is the trade this hardware forces. If photographic finals are wanted '
      'at pace, the rendering should move to a GPU box.\n')
    A('## Self-QA\n')
